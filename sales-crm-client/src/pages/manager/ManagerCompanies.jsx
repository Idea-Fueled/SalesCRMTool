...
export default function ManagerCompanies() {
    ...
    const fetchData = async () => {
        setLoading(true);
        try {
            const res = await getRankedCompanies({ name: search || undefined, limit: 100 });
    ...
    const stats = [
        { label: "Team Companies", value: String(companies.length), color: "bg-red-600 text-white shadow-md shadow-red-100", icon: Building2 },
        ...
        { label: "Inactive", value: String(companies.filter(c => c.status === "Inactive").length), color: "bg-red-600 text-white shadow-md shadow-red-100", icon: XCircle },
    ];
                ...
                {viewMode === "list" ? (
                        ...
                        <table className="w-full text-sm">
                            <thead>
                                <tr className="border-b border-gray-100 bg-gray-50">
                                    {["Rank", "Company", "Industry", "Size", "Status", "Owner", "Revenue Range", "Actions"].map(h => (
                            ...
                            <tbody className="divide-y divide-gray-50">
                                {loading && companies.length === 0 ? (
                                    ...
                                    <tr><td colSpan={7} className="text-center py-20 text-gray-400 font-bold uppercase tracking-widest text-[10px] opacity-60 ">NO companies FOUND</td></tr>
                                ) : (
                                    companies.map((c) => (
                    ...
                    <div className="p-5 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                        {loading && companies.length === 0 ? (
                            ...
                            <div className="col-span-full text-center py-20 text-gray-400 font-bold uppercase tracking-widest text-[10px] opacity-60 ">NO companies FOUND</div>
                        ) : (
                            companies.map((c) => (
                                <CompanyCard
                                    key={c._id}
